...
def start(update, context):
    try:
        ...
        if not context.user_data['registered']:
            ...
            message = Message(update.effective_user.id,
                              text='Вы успешно зарегистрировались! Бот начнет работать 3 мая. Мы пришлем вам уведомление об этом. Спасибо, что готовы помогать школьному сообществу развиваться! ')
            context, message_id = send_message(update, context, message)
    except:
        traceback.print_exc()
        ...
        if not context.user_data.get('registered', None):
            new_messages = Bot.register(Media.TELEGRAM,
                                        ...
                                        user_contact_link='@' + (
                                            update.effective_user.username if update.effective_user.username else ''))
            registered = True
            for m in new_messages['send']:
                if MessageMarks.UNREGISTERED in m.marks:
                    registered = False
                elif MessageMarks.SUCCESSFUL_REGISTRATION in m.marks:
                    context.user_data['registered'] = True
            if not registered:
                for message in new_messages['send']:
                    context, message_id = send_message(update, context, message)
